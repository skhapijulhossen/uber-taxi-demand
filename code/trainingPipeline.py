...
def getData(train_feature_path, test_feature_path) -> tuple:
    try:
        df = pd.read_parquet(train_feature_path)
        X_train = df.drop(columns=["taxi_demand",])
        y_train = df.taxi_demand
        df2 = pd.read_parquet(test_feature_path)
...
def trainXGB(X_train, X_test, y_train, y_test) -> xgb.XGBRegressor:
    try:
        ...
        with mlflow.start_run():
            ...
            mlflow.sklearn.log_model(
                random_search.best_estimator_, 'XGBoost_best_model')
...
if __name__ == '__main__':
    ...
    X_train, X_test, y_train, y_test = getData(
        train_feature_path=r'../data/feature-2022.parquet', test_feature_path=r'../data/feature-2023.parquet')

    best_model = trainXGB(X_train, X_test, y_train, y_test)
